...
BOOL CALLBACK EnumWindowsCallback(HWND hwnd, LPARAM param) {
  ...
  if (data->pid == pid && IsMainWindow(hwnd)) {
    data->occurs.insert(hwnd);
  }
  return TRUE;
}

WindowFinderWin::WindowFinderWin(QObject* parent)
  : QObject(parent), m_process(new QProcess(this)), m_timer(new QTimer(this)),
    m_foundWindow(false) {
...
void WindowFinderWin::SetProgram(const QString& program) {
  m_process->setProgram(program);
}

void WindowFinderWin::SetArguments(const QStringList& arguments) {
  m_process->setArguments(arguments);
}

void WindowFinderWin::Start() {
  m_process->start();
}

void WindowFinderWin::StartFindWindow(QProcess::ProcessState state) {
  qDebug() << QStringLiteral("进程") << m_process->program() <<
      QStringLiteral("运行状态:") << state;
...
void WindowFinderWin::LoadProgramProperty() {
  try {
    ...
    m_className = QString::fromStdString(program_class_name).
        toStdWString();
